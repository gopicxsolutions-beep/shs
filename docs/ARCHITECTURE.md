...
| `place_marketplace_order(product_id, quantity default 1)` (migrations `0057`/`0091`/`0098`/`0111`/`0131`/`0150`/`0153`/`0154`, replaced `decrement_product_stock`) | Single atomic `UPDATE ... WHERE stock >= p_quantity` | Stock check-and-decrement AND the `marketplace_orders` INSERT happen in one `security definer` transaction — not just the stock/price part. `decrement_product_stock` (0008) only verified price and handed it back for the client to insert with in a *separate* round trip; nothing forced a client to actually do that honestly. A direct REST insert could set `amount` to anything with stock never touched (live-confirmed round 125: a real ₹5,000 test product ordered at `amount: 1`), and the old RPC was independently callable with no order at all — any authenticated user could silently zero out any seller's stock as a pure DoS, live-confirmed the same round. `buyer_id`/`buyer_name` are derived from `auth.uid()`/`profiles.name` inside the function, never accepted as parameters. `0153` (user-reported: buying had no quantity concept at all) added `p_quantity` — validated `between 1 and 999`, `amount` stored is the TOTAL (`price x quantity`), and the stock check is `>= p_quantity` so a request for more than remains is refused outright rather than partially fulfilled. **`0153` itself introduced a same-day, self-caught regression**: its `drop function ...; create or replace ...` was written from the `0057` definition, silently dropping five guards four LATER migrations had each added (buyer/seller `profile_is_active` checks — 0091/0098/0111 — a 20-orders/hour rate limit and a self-order block — both 0131 — and the `is_active` delisted-product check — 0150, the entire point of that migration). `0154` restores all five (live-verified each one individually, rolled back) and additionally locks `quantity` in `marketplace_order_locked_fields`/`marketplace_orders_update_seller_or_staff` (0153 added the column but never locked it, so a seller's status-advance PATCH could silently rewrite it) | Returns `success:false` rather than raising, for the ordinary "already sold out"/"not enough stock left" case |
