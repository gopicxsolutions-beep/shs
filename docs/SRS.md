...
| FR-AUTH-4 | A fresh signup's `shg_id` stays null and an approval-pending screen shows until the target SHG's join request is decided. The screen distinguishes four states from the requester's most recent `shg_join_requests` row: pending ("Waiting for approval"), rejected, none yet (never submitted one), and removed (her request WAS approved, but something unlinked her from the SHG afterward, outside the join-request flow — e.g. an admin's SHG reassignment) — the last shows "No longer linked to an SHG" with a fresh SHG picker, not the pending copy | Member |
...
| FR-MKT-6 | A reviewer may edit (rating/comment only) or delete her own review at any time (`marketplace_reviews_update_own`/`_delete_own`). Moderating (deleting) another user's review is staff-only, and excludes a staff member's own review (anti-self-dealing) | All (own review), Staff (others') |
...
| FR-MKT-8 | Buyer sees a seller's UPI ID/payment note on the product detail page and can either open her own UPI app pre-filled to pay manually, or scan an equivalent QR code — no real payment gateway, no order-payment linkage | All |
