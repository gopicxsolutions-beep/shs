...
class _ProfileSetupPageState extends State<ProfileSetupPage> {
  ...
  Widget build(BuildContext context) {
    // An SHG is now required, not optional: every signup starts as a plain
    // 'member' with a pending join request (see AppState.completeProfile
    // Setup's doc comment) — Role Select no longer lets anyone self-declare
    // 'leader' (see role_select_page.dart), so becoming a leader now happens
    // ONLY by whoever approves this same join request choosing to promote
    // her at that moment (see ShgJoinRequestsPage). Without a mandatory SHG
    // pick here, a leader-to-be had no path to ever end up linked to a real
    // SHG at all — the actual bug this requirement exists to close.
    //
    ...
    // that runs through Profile Setup either. The real precondition — an
    // Admin must create at least one SHG (Admin > Manage SHGs) before
    // opening signup to real users — already held before this change; it's
    // simply load-bearing now instead of optional.
